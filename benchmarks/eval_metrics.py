...
nr_metrics = ['clipiqa', 'brisque', 'niqe', 'musiq']
video_metrics  = ['dover']
...
def dover(pred_path, device):
    if os.path.isdir(pred_path):
        base_name = os.path.basename(pred_path)
        output_folder = os.path.join(args.out,"temp")
        os.makedirs(output_folder, exist_ok=True)
        output_path = os.path.join(output_folder, f"{base_name}.mp4")
        img2video(pred_path, output_path)
        video_path = output_path
    elif pred_path.lower().endswith(('.mp4', '.avi', '.mov', '.mkv')):
        video_path = pred_path
    else:
        raise ValueError(f"Unsupported input: {pred_path}")
    
    video_path = os.path.abspath(video_path)
    from DOVER.dover_evaluator import evaluate_and_print_dover as dover_eval
    result = dover_eval(video_path)
    return result 

def create_video_metric(name, device):
    match name:
        case 'dover':
            return partial(dover, device=device)
        case _:
            print(f"Failed to initialize video metric '{name}': {e}")
        

def init_models(metrics, device):
    models = {}
    for name in metrics:
        if name not in video_metrics:
            try:
                models[name] = pyiqa.create_metric(name).to(device).eval()
            except Exception as e:
                print(f"Failed to initialize metric '{name}': {e}")
        else:
            models[name] = create_video_metric(name, device)
           
    print(models.keys())
    return models

def compute_metrics(pred_path, pred_frames, gt_frames, models, device, batch_mode, crop, test_y_channel):
    if batch_mode:
        ...
        for name, model in models.items():
            if name not in video_metrics:
                ...
                # 2025.4.19
                if name == 'dover':
                    value = model(pred_path)  
                    results[name] = value
                else:
                    print(f"Unsupported video metric '{name}'")
                    continue
        return results
        ...
        video_results = {}
        for name, model in models.items():
            if name in video_metrics:
                # 2025.4.19
                if name == 'dover':
                    value = model(pred_path)  # video metrics(dover & ewarp)
                    video_results[name] = value
                else:
                    print(f"Unsupported video metric '{name}'")
                    continue
        ...
        return {k: round(np.mean(v), 4) for k, v in results.items()}|video_results
...
def process(gt_root, pred_root, out_path, metrics, batch_mode, crop, test_y_channel, is_center):
    ...
    pred_files = {os.path.splitext(f)[0]: os.path.join(pred_root, f) for f in os.listdir(pred_root) if not (f.lower().endswith("json") or f.lower().endswith("temp_dover"))}
    ...
    for name in tqdm(pred_names, desc="Evaluating"):
        ...
        try:
            ...
            if has_gt:
                ...
                scores = compute_metrics(pred_path, pred_frames, gt_frames, models, device, batch_mode, crop, test_y_channel)
            else:
                nr_models = {k: v for k, v in models.items() if k not in fr_metrics}
                video_models = {k: v for k, v in models.items() if k in video_metrics}
                if not nr_models and not video_models:
                    print(f"Skipping {name}: GT is not provided and no NR-IQA or video metrics found.")
                    continue
                dummy_gt = pred_frames
                scores = compute_metrics(pred_path, pred_frames, dummy_gt, nr_models, device, batch_mode, crop, test_y_channel)
            print(scores)
            results[name] = scores
            for k in scores:
