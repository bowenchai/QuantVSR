...
def process(pred_root, out_path=None, args=None):

    all_items = [
        video
        for video in os.listdir(pred_root)
        if video != "temp_dover" and video != "temp_ewarp" and (not video.endswith("json"))
    ]
    folders_count = 0
    videos_count = 0
    ...
    if is_folder_dominant:
        input_path = os.path.join(out_path, "temp_ewarp")
    ...
    output_folder = os.path.join(out_path,"temp_ewarp")
