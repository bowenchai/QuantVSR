...
def evaluate_set(input_video_dir,device = 'cuda'):
    ...
    dopt["anno_file"] = None
    dopt["data_prefix"] = input_video_dir
    ...
    try:
        with open(
            f"dover_predictions/val-custom_{input_video_dir.split('/')[-1]}.pkl",
            "rb",
        ) as rf:
    ...
    for i, data in enumerate(tqdm(dataloader, desc="Testing")):
        ...
        results_list.append(rescaled_results['overall'])
...
if __name__ == "__main__":
    ...
    for i, data in enumerate(tqdm(dataloader, desc="Testing")):
        ...
        print(type(rescaled_results["overall"]))
