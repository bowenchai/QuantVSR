...
def main():
    ...
    with open("docs/model_structure.txt", 'w') as f:
        print(f"""
----------------- Model Structure ----------------------
type model: {type(model)}
type model.model: {type(model.model)}
type model.model.diffusion_model: {type(model.model.diffusion_model)}
model.model.diffuison_model tructure:
...
------------------ Quantization -------------------
    ...
    if quant_config['quantize']:
        model.model.diffusion_model = load_Quantmodel(model.model.diffusion_model, quant_config, model.device)
        model.model.diffusion_model.eval()
        print("Quantization Success")
        
    with open("docs/quant_model_structure.txt", 'w') as f:
        print(f"""
----------------- QuantModel Structure ----------------------
type model: {type(model)}
type model.model: {type(model.model)}
type model.model.diffusion_model: {type(model.model.diffusion_model)}
model.model.diffuison_model tructure:
{model.model.diffusion_model}
    ...
    print(yaml.dump(quant_config, default_flow_style=False))

    # seq_name_list = sorted(os.listdir(opt.seqs_path))
    seq_name_list = sorted([v for v in os.listdir(opt.seqs_path) if not v.startswith('.')])
        ...
        with torch.no_grad():
            with precision_scope("cuda"):
                with model.ema_scope():
                    ...
                    for n in trange(len(init_segment_list), desc="Sampling"):
                        ...
                        if im_lq_bs.shape[2] > opt.vqgantile_size or im_lq_bs.shape[3] > opt.vqgantile_size:
                            ...
                            for (im_lq_pch, index_infos), (flow_f, _), (flow_b, _), (fwd_occ, _), (bwd_occ, _) \
                                ...
                                del init_latent, noise, t, flows, masks, flow_f, flow_b, fwd_occ, bwd_occ, x_T, semantic_c
                    ...
                    toc = time.time()
        # if model.CALI:
        #     torch.save(model.cali_datas, f"data/cali_data_pth_30/cali_data_{seq_idx}.pth")
        #     model.cali_datas = []
        #     torch.cuda.empty_cache()
        #     print("Data is saved")
    if model.CALI:
        torch.save(model.cali_datas, f"data/cali_data_udm.pth")
