...
class ImageSpliterTh:
    def __init__(self, im, pch_size, stride, sf=1):
        ...
        self.im_ori = im
        self.im_res = torch.zeros([bs, chn, height*sf, width*sf], dtype=im.dtype, device=im.device).cpu()
        self.pixel_count = torch.zeros([bs, chn, height*sf, width*sf], dtype=im.dtype, device=im.device).cpu()
