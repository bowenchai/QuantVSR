...
class LatentDiffusionVSRTextWT(DDPM):
    ...
    def apply_model(self, x_noisy, t, cond, struct_cond, return_ids=False, save_cali=False, **kwargs):
        ...
        if hasattr(self, "split_input_params"):
            ...
            if self.CALI and save_cali:
                ...
                for k, v in cond.items():
                    cali_data_elem["input"][k] = v
            x_recon = self.model(x_noisy, t, **cond)
            if self.CALI and save_cali:
                cali_data_elem['fp_output'] = x_recon
                cali_data_elem = recursive_to(cali_data_elem, 'cpu')
                self.cali_datas.append(cali_data_elem)
    ...
    def p_mean_variance_canvas(self, x, c, struct_cond, t, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
                        return_x0=False, score_corrector=None, corrector_kwargs=None, t_replace=None, tile_size=64, tile_overlap=32, batch_size=4, tile_weights=None, save_cali=False):
        """
        Aggregation Sampling strategy for arbitrary-size image super-resolution
        """
        assert tile_weights is not None
        ...
        for row in range(grid_rows):
            ...
            for col in range(grid_cols):
                ...
                if len(input_list) == batch_size or col == grid_cols-1:
                    ...
                    struct_cond_input = self.structcond_stage_model(cond_list, t_in[:input_list.size(0)])
                    model_out = self.apply_model(input_list, t_in[:input_list.size(0)], c[:input_list.size(0)], struct_cond_input, return_ids=return_codebook_ids, save_cali=save_cali)
    ...
    def p_sample_canvas(self, x, c, struct_cond, t, guidance_scale=-1.0,
                 ...
                 return_codebook_ids=False, quantize_denoised=False, return_x0=False,
                 temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None, t_replace=None,
                 tile_size=64, tile_overlap=32, batch_size=4, tile_weights=None, save_cali=False):
        b, *_, device = *x.shape, x.device
        b = b // self.num_frames
        outputs = self.p_mean_variance_canvas(x=x, c=c, struct_cond=struct_cond, t=t, clip_denoised=clip_denoised,
                                       return_codebook_ids=return_codebook_ids,
    ...
    def p_sample_loop_canvas(self, cond, struct_cond, shape, guidance_scale=-1.0,
        ...
        if start_T is not None:
            timesteps = min(timesteps, start_T)
        iterator = tqdm(reversed(range(0, timesteps)), desc=" " * 6 + "[bold orange1]Diffusion denoising[/bold orange1]", total=timesteps, leave=False, dynamic_ncols=True) if verbose else reversed(
            range(0, timesteps))
        ...
        for i in iterator:
            ...
            img = self.p_sample_canvas(img, cond, struct_cond, ts, guidance_scale=guidance_scale,
                                ...
                                clip_denoised=self.clip_denoised,
                                quantize_denoised=quantize_denoised, t_replace=t_replace,
                                tile_size=tile_size, tile_overlap=tile_overlap, batch_size=batch_size, tile_weights=tile_weights, save_cali=(i%5==0))
