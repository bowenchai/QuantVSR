...
class InflatedUNetModelDualcondV2(nn.Module):
    ...
    def forward_part1(self, x, timesteps=None, context=None, struct_cond=None, y=None, **kwargs):
        assert (y is not None) == (
            self.num_classes is not None
        ), "must specify y if and only if the model is class-conditional"
        hs = []
        ...
        for module in self.input_blocks:
            ...
            hs.append(h)
        h = self.middle_block(h, emb, context, struct_cond)
        return (h, hs, emb, context, struct_cond, x)

    def forward_part2(self, intermediates):
        h, hs, emb, context, struct_cond, x = intermediates
        for module in self.output_blocks:
            # module.to("cuda:1")
            h = th.cat([h, hs.pop()], dim=1)
            h = module(h, emb, context, struct_cond)
